...
import { debounceTime, distinctUntilChanged, retry, tap, take } from 'rxjs/operators';
...
export class LocationsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  deleteRow(row: any) {
    ...
    const index = this.dataSource.data.findIndex((r: any) => r.locationId === row.locationId);
    if (index > -1) {
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteLocation(row.locationId, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteLocation(rows.locationId, 'response', false).pipe(take(1))
