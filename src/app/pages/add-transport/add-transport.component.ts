...
export class AddTransportComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    const observer = {
      next: (response) => {
        ...
        if (response && (response.id)) {
          ...
          this.router.navigate(['/pages/transports']);
        } else {
          this.utilities.showSnackBar('Unknown Error on create transport', 'OK');
        }
      },
      error: (error) => {
        this.isLoadingResults = false;
    ...
    this.subscriptions.push(this.dataProviderService.createTransport(toUpload, this.row.id)
    .subscribe(observer));
  }
