...
export class OwnersComponent implements OnInit, AfterViewInit, OnDestroy {
  definitions: any = ModelMap.OwnersMap;
  ...
  initColumnsDefs() {
    ...
    let aux;
    if (localStorage.getItem('displayedColumnsInOwnersPage')) {
      this.columnDefs = JSON.parse(localStorage.getItem('displayedColumnsInOwnersPage'));
    } else {
  ...
  toggleColumbn(column?) {
    ...
    localStorage.setItem('displayedColumnsInOwnersPage', JSON.stringify(this.columnDefs));
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteOwner(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteOwner(rows.id, 'response', false).pipe(take(1))
  ...
  loadData(useCache = true) {
    this.utilities.log('requesting owners');
    this.isLoadingResults = true;
    this.subscriptions.push(this.dataProviderService.getAllOwners().subscribe(results => {
      this.isLoadingResults = false;
      this.utilities.log('owners received', results);
  ...
  addRow() {
    ...
    this.utilities.dataTypesModelMaps.owners);
  ...
  export() {
    ...
    this.utilities.exportToXlsx(dataToExport, 'Owners List');
  }
