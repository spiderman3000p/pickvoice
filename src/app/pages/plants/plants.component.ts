...
export class PlantsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initColumnsDefs() {
    ...
    let aux;
    if (localStorage.getItem('displayedColumnsInPlantsPage')) {
      this.columnDefs = JSON.parse(localStorage.getItem('displayedColumnsInPlantsPage'));
    } else {
  ...
  toggleColumbn(column?) {
    ...
    localStorage.setItem('displayedColumnsInPlantsPage', JSON.stringify(this.columnDefs));
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deletePlant(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deletePlant(rows.id, 'response', false).pipe(take(1))
  ...
  applyFilters() {
    ...
    this.dataSource.filterPredicate = (data: Plant, filter: string) => {
  ...
  loadData(useCache = true) {
    this.utilities.log('requesting plants');
    this.isLoadingResults = true;
    this.subscriptions.push(this.dataProviderService.getAllPlants().subscribe(results => {
      this.isLoadingResults = false;
      this.utilities.log('plants received', results);
  ...
  addRow() {
    ...
    this.utilities.dataTypesModelMaps.plants);
  ...
  export() {
    ...
    this.utilities.exportToXlsx(dataToExport, 'Plants List');
  }
