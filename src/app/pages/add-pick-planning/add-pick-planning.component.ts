...
@Component({
  selector: 'app-add-pick-planning',
  templateUrl: './add-pick-planning.component.html',
  styleUrls: ['./add-pick-planning.component.scss']
})
export class AddPickPlanningComponent implements OnInit, OnDestroy {
  definitions = ModelMap.PickPlanningMap;
  form: FormGroup;
  row: any;
  ...
  onSubmit() {
    ...
    const observer = {
      next: (response) => {
        ...
        if (response && (response.id)) {
          ...
          this.router.navigate(['/pages/pickPlannings']);
        } else {
          this.utilities.showSnackBar('Unknown Error on create pick planning', 'OK');
        }
      },
      error: (error) => {
        this.isLoadingResults = false;
    ...
    this.subscriptions.push(this.dataProviderService.createPickPlanning(toUpload, this.row.id)
    .subscribe(observer));
  }
