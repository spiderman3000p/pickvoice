...
@Component({
  selector: 'app-add-lpn-interval',
  templateUrl: './add-lpn-interval.component.html',
  styleUrls: ['./add-lpn-interval.component.scss']
})
export class AddLpnIntervalComponent implements OnInit, OnDestroy {
  definitions = ModelMap.LpnIntervalMap;
  form: FormGroup;
  row: any;
  ...
  constructor(
    ...
    const formControls = {};
    this.row = ModelFactory.newEmptyLpnInterval();
    let validators;
    let value;
  ...
  onSubmit() {
    ...
    const observer = {
      next: (response) => {
        ...
        if (response && (response.id)) {
          ...
          this.router.navigate(['/pages/lpnInterval']);
        } else {
          this.utilities.showSnackBar('Unknown Error on create lpn interval', 'OK');
        }
      },
      error: (error) => {
        this.isLoadingResults = false;
    ...
    this.utilities.log('data to upload', formData);
    this.subscriptions.push(this.dataProviderService.createLpnInterval(formData)
    .subscribe(observer));
  }
