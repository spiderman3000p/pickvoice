...
import { LpnInterval } from '@pickvoice/pickvoice-api';
...
@Component({
  selector: 'app-lpn-interval',
  templateUrl: './lpn-interval.component.html',
  styleUrls: ['./lpn-interval.component.css']
})
export class LpnIntervalComponent implements OnInit, AfterViewInit, OnDestroy {
  definitions: any = ModelMap.LpnIntervalMap;
  dataSource: MatTableDataSource<LpnInterval>;
  dataToSend: LpnInterval[];
  ...
  initColumnsDefs() {
    ...
    let aux;
    if (localStorage.getItem('displayedColumnsInLpnIntervalPage')) {
      this.columnDefs = JSON.parse(localStorage.getItem('displayedColumnsInLpnIntervalPage'));
    } else {
  ...
  toggleColumbn(column?) {
    ...
    localStorage.setItem('displayedColumnsInLpnIntervalPage', JSON.stringify(this.columnDefs));
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteLpnInterval(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteLpnInterval(rows.id, 'response', false).pipe(take(1))
  ...
  applyFilters() {
    ...
    this.dataSource.filterPredicate = (data: LpnInterval, filter: string) => {
  ...
  loadData(useCache = true) {
    this.utilities.log('requesting lpn intervals');
    this.isLoadingResults = true;
    this.subscriptions.push(this.dataProviderService.getAllLpnIntervals().subscribe(results => {
      this.isLoadingResults = false;
      this.utilities.log('lpn intervals received', results);
  ...
  addRow() {
    ...
    this.utilities.dataTypesModelMaps.lpnInterval);
    const dialogRef = this.dialog.open(AddRowDialogComponent, {
      data: {
        map: this.definitions,
        type: IMPORTING_TYPES.LPN_INTERVAL,
        remoteSync: true, // para mandar los datos a la BD por la API
        title: 'Add New Lpn Interval'
      }
    });
    this.subscriptions.push(dialogRef.afterClosed().subscribe(result => {
      this.utilities.log('dialog result:', result);
  ...
  export() {
    ...
    this.utilities.exportToXlsx(dataToExport, 'Lpn Interval List');
  }
