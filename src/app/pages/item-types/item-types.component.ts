...
import { ItemTypeService, ItemType } from '@pickvoice/pickvoice-api';
...
export class ItemTypesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initColumnsDefs() {
    ...
    let aux;
    if (localStorage.getItem('displayedColumnsInItemTypesPage')) {
      this.columnDefs = JSON.parse(localStorage.getItem('displayedColumnsInItemTypesPage'));
    } else {
  ...
  toggleColumn(column?) {
    ...
    localStorage.setItem('displayedColumnsInItemTypesPage', JSON.stringify(this.columnDefs));
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteItemType(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteItemType(rows.id, 'response', false).pipe(take(1))
