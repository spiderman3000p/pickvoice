...
export class AddLocationComponent implements OnInit, OnDestroy {
  definitions = ModelMap.LocationMap;
  form: FormGroup;
  row: any;
  ...
  onSubmit() {
    ...
    const observer = {
      next: (response) => {
        ...
        if (response && (response.id)) {
          ...
          this.utilities.showSnackBar('Unknown Error on create location', 'OK');
        }
      },
      error: (error) => {
        this.isLoadingResults = false;
    ...
    this.subscriptions.push(this.dataProviderService.createLocation(toUpload, this.row.id)
    .subscribe(observer));
  }
