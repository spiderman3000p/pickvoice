...
export class TaskTypeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteTaskType(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteTaskType(rows.id, 'response', false).pipe(take(1))
