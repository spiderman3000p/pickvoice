...
export class QualityStatesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteQualityState(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteQualityState(rows.id, 'response', false).pipe(take(1))
