...
export class DepotsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initColumnsDefs() {
    ...
    let aux;
    if (localStorage.getItem('displayedColumnsInDepotsPage')) {
      this.columnDefs = JSON.parse(localStorage.getItem('displayedColumnsInDepotsPage'));
    } else {
  ...
  toggleColumbn(column?) {
    ...
    localStorage.setItem('displayedColumnsInDepotsPage', JSON.stringify(this.columnDefs));
  ...
  deleteRows(rows: any) {
    ...
    if (Array.isArray(rows)) {
      rows.forEach(row => {
        this.subscriptions.push(this.dataProviderService.deleteDepot(row.id, 'response', false).pipe(take(1))
      ...
      this.subscriptions.push(this.dataProviderService.deleteDepot(rows.id, 'response', false).pipe(take(1))
  ...
  applyFilters() {
    ...
    this.dataSource.filterPredicate = (data: Depot, filter: string) => {
  ...
  loadData(useCache = true) {
    this.utilities.log('requesting depots');
    this.isLoadingResults = true;
    this.subscriptions.push(this.dataProviderService.getAllDepots(this.authService.getPlantId())
    .subscribe(results => {
      this.isLoadingResults = false;
      this.utilities.log('depots received', results);
  ...
  export() {
    ...
    this.utilities.exportToXlsx(dataToExport, 'Depots List');
  }
