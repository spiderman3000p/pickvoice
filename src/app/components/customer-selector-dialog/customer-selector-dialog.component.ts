...
import { Customer } from '@pickvoice/pickvoice-api';
...
@Component({
  ...
  templateUrl: './customer-selector-dialog.component.html',
  styleUrls: ['./customer-selector-dialog.component.css']
})
export class SearchCustomerDialogComponent implements OnInit, AfterViewInit {
  dataSource: MyDataSource<Customer>;
  definitions = ModelMap.CustomerMap;
  displayedColumns = ['customerName', 'customerCode', 'customerNit', 'customerStatus'];
  ...
  getFilterParams(): string {
    ...
    const stringParams = filtersToUse.length > 0 ? filtersToUse.map(filterKey =>
      `${filterKey}-filterType=text;${filterKey}-type=contains;${filterKey}-filter=${formValues[filterKey].toLowerCase()}`)
      .join(';') : '';
  ...
  loadDataPage() {
    ...
    this.dataSource.loadData(IMPORTING_TYPES.CUSTOMERS, `${params}`)
